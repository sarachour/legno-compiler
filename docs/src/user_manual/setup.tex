...
For example, the tag may indicate the board has the model number \tx{<c3>}. 

\noindent\textbf{Important}: This manual will use \tx{<model-number>} to indicate where the model of the board
should be filled in when dispatching commands. Whenever you see the \tx{<model-number>} argument, replace the 
argument with the model number: \tx{c4}, for example.
...
with the device state archive \tx{<model-number>-devstate.zip} and benchmarks 
archive \tx{<model-number>-bmarks.zip}. 
...
This will unpack the relevent databases to the \tx{device-state/hcdc/<model-number>} directory and the benchmark
...
\tx{outputs/legno/unrestricted} directory and peruse the \tx{lgraph-vis} and \tx{lscale-vis} for visualizations of the 
...
benchmark \tx{<bmark>}, remove the following directories:
...
The first command re-runs the \adp{}s on the \hcdc{} device and records the emitted waveforms to the \tx{out-waveforms} directory. 
The second command processes the collected waveforms and plots the data -- the plotted data is written to the \tx{plots} directory.
