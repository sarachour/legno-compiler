...
Download \tx{android.tar.xz} from the above arduino website and run the following to install and open the Arduino IDE:
...
Sometimes, the arduino environment variables don't get set up properly. Add the following to your \tx{.profile} or \tx{.bashrc} file.Execute \tx{whereis arduino} to figure out the directory to assign \tx{ARDUINO_DIR} to (it is \tx{/usr/local/bin} for me).
